...
//     .catch(err => {
//       console.error(`${err} 💥💥💥`);
//       renderError(`Somrthing went wrong 💥💥 ${err.message}. Try again!`);
//     })
//     .finally(() => {
//       countriesContainer.style.opacity = 1;
//     });
...
const whereAmI = async function () {
  try {
    ...
    const res = await fetch(
      `https://restcountries.eu/rest/v2/name/${dataGeo.country || 'Thailand'}`
    );
    if (!res.ok) throw new Error('Problem getting country');
    const [data] = await res.json();
    renderCountry(data);

    return `You are in ${dataGeo.city}, ${dataGeo.country || 'Thailand'}`;
  } catch (err) {
    console.error(err);
    renderError(`💥 ${err.message}!`);

    // Reject ptomise returned from async function
    throw err;
  }
};
...
/*
// Promise.all (Running Asynchronous in Parallel)
const getJSON = function (url, errorMsg = 'Something went wrong') {
  return fetch(url).then(response => {
    if (!response.ok) throw new Error(`${errorMsg} (${response.status})`);
    return response.json();
...
get3Countries('Thailand', 'Japan', 'Vietnam');
*/
...
(async function () {
  const response = await Promise.race([
    getJSON(`https://restcountries.eu/rest/v2/name/macao`),
